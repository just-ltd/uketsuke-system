...
const EXTRACTION_PROMPT = `以下のSlack会話から、受付表の項目を抽出してJSON形式で出力してください。

## 抽出する項目
- formTitle: 受付表タイトル（下記の分類ルールに従って決定）
- jisshiDate: 実施日
...
## formTitle（受付表タイトル）の分類ルール
会話の作業内容から、以下の分類に従ってタイトルを決定してください。

種別一覧:
- "X線探査" : X線撮影に関する作業
- "レーダー探査" : レーダー探査、電磁波レーダーに関する作業
- "地中レーダー探査" : 地中レーダー探査、埋設配管探査に関する作業
- "コア・アンカー・斫り工事" : コア削孔、アンカー、斫りに関する作業

複数の種別が含まれる場合は「・」で繋いでください。
例: "X線・レーダー探査受付表"、"X線・コア・アンカー・斫り工事受付表"

formTitleの値は必ず「○○受付表」の形式にしてください。
デフォルト（判断できない場合）: "レーダー探査受付表"
